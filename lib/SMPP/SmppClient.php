...
use Viebig\PhpSmpp\SMPP\SMPP;
use Viebig\PhpSmpp\SMPP\SmppAddress;
use Viebig\PhpSmpp\SMPP\SmppDeliveryReceipt;
use Viebig\PhpSmpp\SMPP\SmppException;
use Viebig\PhpSmpp\SMPP\SmppPdu;
use Viebig\PhpSmpp\SMPP\SmppSms;
use Viebig\PhpSmpp\SMPP\SmppTag;
use Viebig\PhpSmpp\SocketTransport\SocketTransport;
...
class SmppClient {
	...
	public function sendSMS(SmppAddress $from, SmppAddress $to, $message, $tags = null, $dataCoding = SMPP::DATA_CODING_DEFAULT, $priority = 0x00, $scheduleDeliveryTime = null, $validityPeriod = null) {
		...
		if ($doCsms) {
			if (self::$csms_method == SmppClient::CSMS_PAYLOAD) {
				...
				return $this->submitSm($from, $to, null, (empty($tags) ? array($payload) : array_merge($tags, $payload)), $dataCoding, $priority, $scheduleDeliveryTime, $validityPeriod);
			} else if (self::$csms_method == SmppClient::CSMS_8BIT_UDH) {
				$seqnum = 1;
				foreach ($parts as $part) {
					$udh = pack('cccccc', 5, 0, 3, substr($csmsReference, 1, 1), count($parts), $seqnum);
					$res = $this->submitSm($from, $to, $udh . $part, $tags, $dataCoding, $priority, $scheduleDeliveryTime, $validityPeriod, (SmppClient::$sms_esm_class | 0x40));
					$seqnum++;
				}
				return $res;
			} else {
	 ...
	 * @return string message id
	 */
	protected function submitSm(SmppAddress $source, SmppAddress $destination, $short_message = null, $tags = null, $dataCoding = SMPP::DATA_CODING_DEFAULT, $priority = 0x00, $scheduleDeliveryTime = null, $validityPeriod = null, $esmClass = null) {
		if (is_null($esmClass)) {
	...
	protected function sendCommand($id, $pduBody) {
		...
		$this->sendPDU($pdu);
		$response = $this->readpduResp($this->sequence_number, $pdu->id);
		if ($response === false) {
	 ...
	 * @throws SmppException
	 */
	protected function readpduResp($seq_number, $command_id) {
